...
func (c *Client) DownloadAttachment(ctx context.Context, signedURL string) ([]byte, int, error) {
	...
	parsedURL, err := urlpolicy.ValidateAppleOrSignedHTTPS(signedURL, "review attachment download URL")
	if err != nil {
		return nil, 0, err
	}
	...
	response, err := urlpolicy.ClientWithoutRedirects(c.httpClient).Do(request)
	if err != nil {
