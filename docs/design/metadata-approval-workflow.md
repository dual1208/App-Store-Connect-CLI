...
Issue #1717 asks for automation-friendly metadata changes with a durable plan, explicit approval, and a guarded apply. Current `metadata apply --dry-run` computes a useful plan but does not persist a review artifact or bind approval to the exact plan that was reviewed.
