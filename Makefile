...
test:
	...
	./scripts/run-go-tests-isolated.sh -v ./...
...
test-parallel:
	...
	./scripts/run-go-tests-isolated.sh -v -count=1 $(if $(PARALLEL),-p=$(PARALLEL)) ./...

# Run tests with coverage
.PHONY: test-coverage
test-coverage:
	@echo "$(BLUE)Running tests with coverage...$(NC)"
	./scripts/run-go-tests-isolated.sh -coverprofile=coverage.out ./...
...
.PHONY: check-docs
check-docs: check-command-docs check-repo-docs check-website-docs check-openapi
