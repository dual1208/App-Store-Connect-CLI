...
func TestDownloadDSYMRefusesRedirectWithoutForwardingReferer(t *testing.T) {
	...
	restore := SetDSYMHTTPClient(&http.Client{Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
		clone := req.Clone(req.Context())
		clone.URL.Scheme = sourceURL.Scheme
		clone.URL.Host = sourceURL.Host
		return baseTransport.RoundTrip(clone)
	})})
	t.Cleanup(restore)
	_, err = downloadDSYM(
		context.Background(),
		"https://download.apple.com/symbols?Signature=signed-secret",
...
func TestDownloadDSYMSanitizesTransportErrorAndPreservesCause(t *testing.T) {
	...
	_, err := downloadDSYM(
		context.Background(),
		"https://example.cloudfront.net/file?Signature=signed-secret",
		filepath.Join(t.TempDir(), "symbols.zip"),
	)
	if err == nil {
...
func TestDownloadDSYMDoesNotWriteRedirectOrErrorBody(t *testing.T) {
	...
	if _, err := downloadDSYM(context.Background(), "https://download.apple.com/file", dest); err == nil {
		t.Fatal("expected HTTP error")
	}
}

func TestDownloadDSYMRejectsInsecureUserinfoAndUntrustedURLs(t *testing.T) {
	dest := filepath.Join(t.TempDir(), "symbols.zip")
	for _, rawURL := range []string{
		"http://download.apple.com/file",
		"https://user:secret@download.apple.com/file",
		"https://example.invalid/file",
	} {
		if _, err := downloadDSYM(context.Background(), rawURL, dest); err == nil {
