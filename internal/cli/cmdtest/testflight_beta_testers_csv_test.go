...
func TestTestFlightBetaTestersImport_CreateAssignAndInvite(t *testing.T) {
	...
	stdout, stderr := captureOutput(t, func() {
		if err := root.Parse([]string{"testflight", "testers", "import", "--app", "app-1", "--input", csvPath, "--invite", "--confirm"}); err != nil {
			t.Fatalf("parse error: %v", err)
		}
		if err := root.Run(context.Background()); err != nil {
			t.Fatalf("run error: %v", err)
...
func TestTestFlightBetaTestersImport_TitleCasedHeaderAndSemicolonGroups(t *testing.T) {
	setupAuth(t)
	...
	csvPath := filepath.Join(t.TempDir(), "title-cased.csv")
	csvBody := "" +
...
func TestTestFlightBetaTestersImport_LegacyHeaderlessFormat(t *testing.T) {
	setupAuth(t)
