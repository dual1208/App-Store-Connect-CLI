...
dSYM files are used for crash symbolication. Each build bundle that includes
symbols will have a dSYM download URL.
...
// format. Falls back to buildID-based names.
...
func downloadDSYM(ctx context.Context, rawURL, destPath string) (int64, error) {
	parsedURL, err := urlpolicy.ValidateAppleOrSignedHTTPS(rawURL, "dSYM download URL")
	if err != nil {
		return 0, err
	}
	...
	resp, err := urlpolicy.ClientWithoutRedirects(client).Do(req)
	if err != nil {
		return 0, urlsanitize.NewTransportError(
