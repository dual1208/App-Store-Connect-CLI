...
import (
	...
	rootcmd "github.com/dual1208/App-Store-Connect-CLI/cmd"
	"github.com/dual1208/App-Store-Connect-CLI/internal/auth"
	authcli "github.com/dual1208/App-Store-Connect-CLI/internal/cli/auth"
...
func TestAuthStatusShowsEnvFallbackNote(t *testing.T) {
	...
	if !strings.Contains(stdout, "Stored file credentials are preferred") {
...
func TestAuthStatusShowsStorageLocation(t *testing.T) {
	...
	if !strings.Contains(stdout, "Credential storage: Private Config File") {
...
func TestAuthLoginUsesConfiguredFilePath(t *testing.T) {
	tempDir := t.TempDir()
	keyPath := filepath.Join(tempDir, "AuthKey.p8")
	writeECDSAPEM(t, keyPath)
