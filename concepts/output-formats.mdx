...
`asc testflight review view`, and `asc testflight review edit`.

Redaction affects only what is printed. Passwords you pass with
`--demo-account-password` are still sent to App Store Connect.
...
<Warning>
  ...
  If a script depended on reading the password back, add `--include-sensitive`
