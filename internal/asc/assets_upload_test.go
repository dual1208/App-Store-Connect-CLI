...
func TestUploadAssetFromFileRejectsInsecureAndUserinfoURLs(t *testing.T) {
	...
	for _, rawURL := range []string{
		"http://upload.apple.com/part",
		"https://user:secret@upload.apple.com/part",
	} {
		if err := UploadAssetFromFile(context.Background(), file, 4, []UploadOperation{{
...
func TestUploadAssetFromFileUploadsChunks(t *testing.T) {
	...
	defer server.Close()
	t.Cleanup(swapUploadTransport(t, server.Client().Transport))
