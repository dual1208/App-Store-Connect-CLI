...
<ParamField path="ASC_PROFILE" type="string">
  Use a named authentication profile from the private config file
</ParamField>
...
<Note>
  When `ASC_STRICT_AUTH=true`, the CLI fails if credentials are complete in both the selected file profile and the environment.
</Note>
