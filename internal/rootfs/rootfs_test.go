...
func TestAppendFileRefusesSymlinkAndLeavesModeIntact(t *testing.T) {
	...
	if err := os.Symlink(sentinelPath, filepath.Join(dir, "audit.log")); err != nil {
		t.Fatalf("Symlink() error = %v", err)
	}

	root := mustRoot(t, dir)
	if err := root.AppendFile("audit.log", []byte("entry\n"), 0o600); !errors.Is(err, ErrSymlink) {
...
func TestAppendFileAppendsInRoot(t *testing.T) {
	...
	if err := root.AppendFile("audit.log", []byte("one\n"), 0o600); err != nil {
		t.Fatalf("AppendFile() error = %v", err)
	}
	if err := root.AppendFile("audit.log", []byte("two\n"), 0o600); err != nil {
		t.Fatalf("AppendFile() error = %v", err)
	}
	if got := mustRead(t, filepath.Join(dir, "audit.log")); got != "one\ntwo\n" {
		t.Fatalf("content = %q", got)
	}
}
...
func TestAppendFileRefusesMultiplyLinkedFile(t *testing.T) {
	...
	linkedPath := filepath.Join(dir, "audit.log")
...
func TestAppendFileDoesNotEscapeWhenParentIsSwappedAfterValidation(t *testing.T) {
	...
	dir := t.TempDir()
	nested := filepath.Join(dir, "nested")
	mustWrite(t, filepath.Join(nested, "audit.log"), "trusted\n")
	external := t.TempDir()
	externalLog := filepath.Join(external, "audit.log")
	...
	err := root.AppendFile(filepath.Join("nested", "audit.log"), []byte("attacker\n"), 0o600)
	if err != nil {
		t.Fatalf("AppendFile() error = %v", err)
	}
	...
	if got := mustRead(t, filepath.Join(dir, "nested-original", "audit.log")); got != "trusted\nattacker\n" {
...
func TestAppendFileRejectsFinalSymlinkSwappedAfterValidation(t *testing.T) {
	...
	err := root.AppendFile("audit.log", []byte("attacker\n"), 0o600)
	if err == nil {
