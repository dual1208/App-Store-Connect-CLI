...
Automatic generation uses archive metadata only and does not touch local
credential stores or App Store Connect. Manual generation is macOS-only and may inspect
...
External scripting runtimes or Xcode UI export violate ASC's single-binary and noninteractive
