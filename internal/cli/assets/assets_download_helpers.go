...
const (
	...
	assetDownloadUserAgent    = "asc/asset-download"
)
...
func resolveImageAssetDownloadURL(asset *asc.ImageAsset, fileName string) (string, error) {
	...
	if _, err := urlpolicy.ValidateAppleOrSignedHTTPS(resolved, "screenshot download URL"); err != nil {
		return "", err
	}
...
func downloadURLToFile(ctx context.Context, rawURL string, outputPath string, overwrite bool) (int64, string, error) {
	...
	if _, err := urlpolicy.ValidateAppleOrSignedHTTPS(rawURL, "asset download URL"); err != nil {
		return 0, "", err
	}
...
func downloadURLToFileOnce(ctx context.Context, rawURL string, outputPath string, overwrite bool) (int64, string, error) {
	...
	resp, err := urlpolicy.ClientWithoutRedirects(http.DefaultClient).Do(req)
	if err != nil {
		return 0, "", err
	}
