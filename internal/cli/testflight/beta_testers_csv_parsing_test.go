...
func TestReadBetaTestersCSV_TitleCasedHeaderAliases(t *testing.T) {
	path := filepath.Join(t.TempDir(), "title-cased-header.csv")
	body := strings.Join([]string{
...
func TestReadBetaTestersCSV_LegacyHeaderlessRows(t *testing.T) {
	path := filepath.Join(t.TempDir(), "legacy-no-header.csv")
	body := strings.Join([]string{
