...
func TestRun_AlternativeDistributionHelpIncludesEUAddendumOperationalGuidance(t *testing.T) {
	resetReportFlags(t)
...
func TestRootCommand_UsageGroupsSubcommands(t *testing.T) {
	...
	if !strings.Contains(usage, "  auth:") || !strings.Contains(usage, "  doctor:") || !strings.Contains(usage, "  docs:") {
...
func TestRun_AuthTokenRejectsPermissiveKeyFile(t *testing.T) {
	...
	if !strings.Contains(stderr, "private key file must have mode 0600") {
