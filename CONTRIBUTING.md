...
Credentials are stored in a private config file at `~/.asc/config.json`. A repo-local
`./.asc/config.json` is also supported.
...
ASC_BYPASS_KEYCHAIN=1 make test  # Compatibility switch plus isolated file stores
...
maintainers should add any missing labels during first triage, and new issues should not be left
