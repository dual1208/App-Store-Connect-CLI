...
func TestBuildsDsymsBuildSelectorIgnoresDefaultAppID(t *testing.T) {
	...
	http.DefaultTransport = roundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		case req.Method == http.MethodGet && req.URL.Path == "/v1/builds/build-1" && req.URL.RawQuery == "include=buildBundles":
			body := `{"data":{"type":"builds","id":"build-1","attributes":{"version":"42"}},"included":[{"type":"buildBundles","id":"bundle-1","attributes":{"bundleId":"com.example.app","dSYMUrl":"https://downloads.apple.com/app.dSYM.zip"}}]}`
			return &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(strings.NewReader(body)),
				Header:     http.Header{"Content-Type": []string{"application/json"}},
