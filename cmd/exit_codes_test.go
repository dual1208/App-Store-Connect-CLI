...
// status in the exit code once retries are
...
func isolatedCLITestEnv(configPath string) []string {
	env := filterEnvVars(
		...
		"ASC_STRICT_AUTH",
		"ASC_APP_ID",
		"ASC_WEB_SESSION_CACHE_DIR",
		"ASC_WEB_PASSWORD_STORE_DIR",
	)
	return append(
		env,
		...
		"ASC_WEB_SESSION_CACHE_DIR="+filepath.Join(filepath.Dir(configPath), "web-sessions"),
		"ASC_WEB_PASSWORD_STORE_DIR="+filepath.Join(filepath.Dir(configPath), "web-passwords"),
	)
}
