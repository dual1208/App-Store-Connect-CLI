...
func TestDownloadAttachmentReturnsStatusAndBody(t *testing.T) {
	...
	body, status, err := client.DownloadAttachment(context.Background(), "https://appstoreconnect.apple.com/ok")
	if err != nil {
	...
	_, status, err = client.DownloadAttachment(context.Background(), "https://appstoreconnect.apple.com/expired")
	if err == nil {
...
func TestDownloadAttachmentErrorDoesNotLeakSignedURLTokens(t *testing.T) {
	...
	signedURL := "https://example.cloudfront.net/download?token=very-secret&X-Amz-Signature=abc123"
...
func TestDownloadAttachmentRejectsUntrustedHost(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "not an allowed Apple or signed CDN host") {
		t.Fatalf("unexpected error: %v", err)
	}
}

func TestDownloadAttachmentRejectsUserinfo(t *testing.T) {
	client := &Client{httpClient: &http.Client{}}
	_, _, err := client.DownloadAttachment(context.Background(), "https://user:secret@appstoreconnect.apple.com/download")
