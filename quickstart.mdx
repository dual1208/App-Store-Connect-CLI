...
<Steps>
  ...
  <Step title="Verify storage">
    ...
    ```
    Credential storage: Private Config File
    Location: ~/.asc/config.json

    Stored credentials:
    Name    Key ID      Default  Stored In
    MyApp   ABC123DEFG  yes      config
    ```
  </Step>
</Steps>

<Note>
  Credentials are stored only in a private config file. The directory must be mode `0700`; the config and `.p8` files must be ordinary current-user-owned files with mode `0600`.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Automation" icon="gear" href="/guides/automation">
    Create repeatable App Store Connect workflows
  </Card>
...
* **GitHub Issues**: [Report bugs or request features](https://github.com/dual1208/App-Store-Connect-CLI/issues)
* **Discussions**: [Ask questions and share workflows](https://github.com/dual1208/App-Store-Connect-CLI/discussions)
