...
import (
	...
	"github.com/dual1208/App-Store-Connect-CLI/internal/ascterritory"
	"github.com/dual1208/App-Store-Connect-CLI/internal/auth"
...
func resolveCredentialsForProfile(profileOverride string) (resolvedCredentials, error) {
	...
	// Priority 1: stored file credentials.
...
func configCredentialMetadataSummaries(cfg *config.Config) []credentialMetadataSummary {
	...
	summaries := make([]credentialMetadataSummary, 0, len(cfg.Keys)+1)
	seen := make(map[string]struct{}, len(cfg.Keys)+1)
...
func validateNextURL(next string) error {
	...
	if parsed.User != nil || parsed.Fragment != "" {
		return fmt.Errorf("--next must not contain userinfo or a fragment")
	}
	return nil
}
...
func GetASCClient() (*asc.Client, error) {
	// Show a subtle spinner for file and environment credential resolution.
...
// without loading private key material from disk.
